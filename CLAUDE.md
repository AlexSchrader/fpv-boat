...
- **`pan_tilt_control.py`** — camera pan/tilt head-tracking via a **PCA9685** driving 2 SG90 servos. The viewer streams head yaw/pitch (degrees, neutral captured at VR entry) over `/ws/control`; `pan_tilt.set_head()` maps it to servo angles via the pure, unit-tested `head_to_servo()` (tilt clamped to 45–135° so the camera can't hit the hull). **No-op without `adafruit_servokit` / the board** — tracks angle only. Centers on control-link drop. `/telemetry` exposes `pan_angle`/`tilt_angle`. **Watch the I2C 0x40 collision with the INA219** — readdress one (`PAN_TILT_I2C_ADDR` / `BATTERY_I2C_ADDR`).
