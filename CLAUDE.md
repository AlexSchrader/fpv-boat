...
- **Field AP mode** (`setup_ap.sh`): away from the house the Pi broadcasts its own WiFi (NetworkManager native AP, `fpv-ap` profile, BrosTrend AC5L as `wlan1`, Pi at `192.168.4.1`, headset connects direct). Onboard `wlan0` keeps the home-network profile untouched — don't reconfigure it.
...
- **`webrtc_stream.py`** — the server. aiohttp app, HTTPS. Owns the `Picamera2` object and everything camera-related. Routes:
  ...
  - `/telemetry` — JSON status: flat legacy fields (recording, disk, CPU, battery_*, pan/tilt) **plus nested validity-aware blocks** `battery`/`gps`/`compass`/`speed` — each sensor degrades to `valid: false` on its own; the HUD never guesses whether a zero is real. `speed` is an indirection (GPS now, accel-fused later) so the source can change without touching the HUD.
...
- **`gps_control.py`** — GPS over UART (`/dev/ttyAMA0` @38400, u-blox M10). Background daemon thread parses NMEA (GGA+RMC via `pynmea2`); `read()` returns fix/sats/hdop/lat/lon/`sog_mph`/cog with **hard validity**: no fix or >3 s stale → `valid: False` and coords are `None`, never 0.000000. Pure helpers (`fix_from`, `knots_to_mph`) are unit-tested. No-op without pyserial/the port.
- **`compass_control.py`** — heading from a **QMC5883P at I2C 0x2C** (NOT the QMC5883L at 0x0D that most libraries target — that mismatch is why off-the-shelf code reads nothing; chip-ID verified at init). Own register driver via smbus2. **Hard-iron calibration required** (`python3 compass_control.py calibrate`, persists to `~/.fpv-boat-compass.json`; HUD shows `UNCAL` until done). `COMPASS_DECLINATION_DEG` (default −9, Raleigh). Tilt-naive. HUD shows it as HDG, distinct from GPS COG.
- **`imu_control.py`** — accel/gyro from an **MPU-6050 (GY-521) at I2C 0x68** (WHO_AM_I verified; chip boots asleep — init must clear PWR_MGMT_1 or all registers read 0). 50 Hz daemon thread with EMA-smoothed **pitch/roll**; the server passes them to `compass.read(tilt=…)` for a **tilt-compensated HDG**. `/telemetry` exposes a validity-aware `imu` block. Pure `pitch_roll_from_accel()` unit-tested. No-op without smbus2/the sensor.
...
**In hand and wired:** Pi Zero 2 W, Arducam Camera Module 3 Wide (CSI ribbon — has unseated twice; reseat powered-off + reboot on `IndexError` from `Picamera2()`), L298N + both motors (bench-tested), ShareGoo LED kit on GPIO17 (working), PCA9685 + 2 SG90 pan/tilt (working), MP1584EN bucks (5 V rail live), INA219 battery sensor, u-blox M10-25Q GPS (UART), QMC5883P compass (I2C 0x2C), 2S LiPo. **⚠️ The PCA9685 and INA219 both default to I2C 0x40 — the PCA9685's A0 pad must be bridged (→0x41, `PAN_TILT_I2C_ADDR=0x41`).**
**Not yet in the loop:** DPDT failsafe switch (required before water), rear reverse-light LEDs (GPIO22 reserved), PG7 glands / IP65 boxes (waterproofing), BrosTrend USB WiFi adapter. Water sensor not needed — tapping the boat's stock hull sensor.
